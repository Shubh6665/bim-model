...
export function OwnerPendingAdminsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80">
      <div className="bg-card border border-border rounded-xl w-full max-w-2xl mx-4">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h3 className="text-foreground font-semibold">Manage Pending Company Admins</h3>
        ...
        <div className="p-4 max-h-[70vh] overflow-auto">
          ...
          {!loading && entries.length === 0 && (
            <div className="text-muted-foreground">No pending admin requests.</div>
          )}
          {!loading && entries.length > 0 && (
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {entries.map((e) => (
                  <tr key={`${e.email}|${e.company}`} className="border-t border-border">
                    <td className="py-2 text-foreground">{e.email}</td>
                    <td className="py-2 text-foreground">{e.company}</td>
                    ...
                    <td className="py-2 text-right">
                      <button
                        className="px-3 py-1 rounded bg-green-600 hover:bg-green-500 text-foreground mr-2 disabled:opacity-60"
                        onClick={() => act(e.email, e.company, 'approve')}
                        disabled={busyKey === `${e.email}|${e.company}`}
                      >Approve</button>
                      <button
                        className="px-3 py-1 rounded bg-red-600 hover:bg-red-500 text-foreground disabled:opacity-60"
        ...
        <div className="p-4 border-t border-border text-right">
          <button onClick={onClose} className="px-4 py-2 rounded bg-muted hover:bg-muted text-foreground">Close</button>
        </div>
      </div>
    </div>
  );
